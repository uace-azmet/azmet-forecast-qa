...
Seems like too many 0s for solar radiation, and none are marked as needing review.
...
daily_ts_sub <-
  daily_ts |> 
  filter(meta_station_name %in% c("Bowie", "Harquahala", "Tucson", "Maricopa")) 
daily_ts_sub |> autoplot(temp_air_meanC)
```

```{r}
daily_ts_sub |> gg_season(temp_air_meanC)
```

```{r}
daily_ts_sub |> 
  ACF(temp_air_meanC, lag_max = 6*30) |>
  autoplot() + 
  scale_x_continuous()

```

`STL()` can't handle missing values.
Need to choose uninterrupted timeseries

```{r}
daily_ts_sub |> 
  model(STL(temp_air_meanC ~ season(window = "periodic")))
```

```{r}
daily_ts_sub |> features(temp_air_meanC, feature_set(tags = "stl"))
```
